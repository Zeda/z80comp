...
class Path:
  def __init__(self,i=0,size=0,speed=0,code=[],req=[],vars=[]):
    self.index=i
    self.size=size
    self.speed=speed
    self.code=code
  ...
  def copy(self):
    ...
    for i in self.code:
      l+=[i.copy()]
    return Path(self.index,self.size,self.speed,l,self.requires[0:],self.vars[0:])
  def __eq__(self,p):
    if PATH_SORT_MODE==PATH_SORT_MODE_SCORE:
      return (self.size*SIZE_MOD+self.speed*SPEED_MOD)==(p.size*SIZE_MOD+p.speed*SPEED_MOD)
    else:
...
def compile(src,c):
  ...
  while paths[0].index<len(src):
    ...
    paths=paths[1:]
    s=src[pth.index]
    p=c.execute("SELECT code,size,speed,input,output,destroys,requires,state FROM z80 WHERE ir=?",(s,)).fetchall()
    if p==[]:
      if s.endswith("\\~"):
        p0=c.execute("SELECT code,size,speed,input,output,destroys,requires,state FROM z80 WHERE ir=?",("\\sto",)).fetchall()
        s="var_"+s[0:-3]
        if s not in pth.vars:
          pth.vars+=[s]
      elif s.endswith("GotoIf("):
        if s.startswith("0 = "):
          p0=c.execute("SELECT code,size,speed,input,output,destroys,requires,state FROM z80 WHERE ir=?",("0 = GotoIf(",)).fetchall()
          s=s[4:-7]
        elif s.startswith("0 != "):
          p0=c.execute("SELECT code,size,speed,input,output,destroys,requires,state FROM z80 WHERE ir=?",("0 != GotoIf(",)).fetchall()
          s=s[5:-7]
        else:
          p0=c.execute("SELECT code,size,speed,input,output,destroys,requires,state FROM z80 WHERE ir=?",("GotoIf(",)).fetchall()
          s=s[0:-7]
      elif isnum(src[pth.index]):
        p0=c.execute("SELECT code,size,speed,input,output,destroys,requires,state FROM z80 WHERE ir=?",("\\constant",)).fetchall()
      else:
        p0=c.execute("SELECT code,size,speed,input,output,destroys,requires,state FROM z80 WHERE ir=?",("\\var",)).fetchall()
        s="var_"+s
        if s not in pth.vars:
          pth.vars+=[s]
      for i in p0:
    ...
    for i in p:
      ...
      if i[3]=='':
        ...
        #if the operation is commutative, then we should try to rearange the inputs!
        #For binary ops, this doubles the space we are trying, but may improve results
        codes=[j.copy()]
        if iscommutative(s):
          j.code[-2:]=j.code[-2:][::-1]
          codes+=[j.copy()]
          ...
          for ins in i[3].split(","):
            ...
            for codo in cod.out:
              if codo in op.destroys:
                #If cod.out is HL or DE and op.destroys doesn't contain the other, we can use `ex de,hl`
                #Otherwise, just push \ pop
                if codo.endswith("HL") and "DE" not in op.destroys:
                  op.destroys+=["DE"]
                  op.code="ex de,hl \ "+op.code+" \ ex de,hl"
                  j.size+=2
                  j.speed+=8
                elif codo.endswith("DE") and "HL" not in op.destroys:
                  op.destroys+=["HL"]
                  op.code="ex de,hl \ "+op.code+" \ ex de,hl"
                  j.size+=2
                  j.speed+=8
                  ...
                  #Here we push \ pop
                  #However, if cod has gluecode to pass registers, we might be able to get rid of it!
                  if codo.endswith("ld b,h \ ld c,l") or codo.endswith("ld c,l \ ld b,h"):
                    cod.code=cod.code[-15:]
                    op.code="push hl \ "+op.code+" \ pop bc"
                    j.speed+=13
                  elif codo.endswith("ld b,d \ ld c,e") or codo.endswith("ld c,e \ ld b,d"):
                    cod.code=cod.code[-15:]
                    op.code="push de \ "+op.code+" \ pop bc"
                    j.speed+=13
                  elif codo.endswith("ld d,h \ ld e,l") or codo.endswith("ld e,l \ ld d,h"):
                    cod.code=cod.code[-15:]
                    op.code="push hl \ "+op.code+" \ pop de"
                    j.speed+=13
                  elif codo.endswith("ld h,b \ ld l,c") or codo.endswith("ld l,c \ ld h,b"):
                    cod.code=cod.code[-15:]
                    op.code="push bc \ "+op.code+" \ pop hl"
                    j.speed+=13
                  elif codo.endswith("ld d,b \ ld e,c") or codo.endswith("ld e,c \ ld d,b"):
                    cod.code=cod.code[-15:]
                    op.code="push bc \ "+op.code+" \ pop de"
                    j.speed+=13
                  elif codo.endswith("ld h,d \ ld l,e") or codo.endswith("ld l,e \ ld h,d"):
                    cod.code=cod.code[-15:]
                    op.code="push de \ "+op.code+" \ pop hl"
                    j.speed+=13
                  else:
                    op.code="push "+codo.lower()+" \ "+op.code+" \ pop "+codo.lower()
                    j.size+=2
                    j.speed+=13
...
def astoptimize(n):
  ...
  #For commutative operations, put the input with fewest children on the right, or constant inputs
  for i in range(n.desc[0]-1):
    if iscommutative(n.value):
      if n.child(i).desc[0]<n.child(i+1).desc[0] or isnum(n.child(i).value):
        a=n.child(i)
        n.children[i]=n.child(i+1)
        n.children[i+1]=a
  #Perform some replacements. ex, 2*x ==> x<<1
  if n.value=="+" and n.child(0).value=="*" and n.child(1).value=="*":
    ...
    p=c.execute("SELECT ir FROM z80 WHERE ir=?",(n.child(1).value+" *",)).fetchone()
    p=None
    if p!=None:
...
def movreg(o,i):
  ...
  if o.endswith(i):
    return ''
  if (o.endswith("HL") and i.endswith("DE")) or (o.endswith("DE") and i.endswith("HL")):
    return " \ ex de,hl"
  elif o.endswith("HL") and i.endswith("BC"):
...
def astcompile(n,outp=''):
  ...
  p=c.execute("SELECT code,input,output,destroys FROM z80 WHERE ir=? AND output LIKE ? ORDER BY size*size*36+speed*speed ASC",(n.value,"%"+outp+"=%",)).fetchall()
  if p==[]:
    p=c.execute("SELECT code,input,output,destroys FROM z80 WHERE ir=? ORDER BY size*size*36+speed*speed ASC",(n.value,)).fetchall()
    if p==[]:
      if isnum(n.value):
        p=c.execute("SELECT code,input,output,destroys FROM z80 WHERE ir=? AND output LIKE ? ORDER BY size*size*36+speed*speed ASC",("constant","%"+outp+"=%",)).fetchall()
      else:
        p=c.execute("SELECT code,input,output,destroys FROM z80 WHERE ir=? AND output LIKE ? ORDER BY size*size*36+speed*speed ASC",("var","%"+outp+"=%",)).fetchall()
      if p==[]:
        raise Exception(n)
  p=p[0]
  ...
  while k<len(n.children):
    n.children[k]=astcompile(n.child(k),ins[k].strip().split("=")[0][-2:])
  ...
  if len(inp)==1:
    ...
    if (o1[0][-2:] in n.child(1).destroys) or (o1[0]==o2[0]):
      f=(o1[0][-2:]=="HL" and "DE" not in n.child(1).destroys) or (o1[0][-2:]=="DE" and "HL" not in n.child(1).destroys)
      if f:
        n.code+=" \ ex de,hl"
      else:
      ...
      m=movreg(o2[0],i2[0])
      if (("ld b," in m) or "ld c," in m) and ("BC" not in n.destroys):
        n.destroys+=["BC"]
      if (("ld d," in m) or "ld e," in m) and ("DE" not in n.destroys):
        n.destroys+=["DE"]
      if (("ld h," in m) or "ld l," in m) and ("HL" not in n.destroys):
        n.destroys+=["HL"]
      if (("ld ixh," in m) or "ld ixl," in m) and ("IX" not in n.destroys):
        n.destroys+=["IX"]
...
if 2*len(vars)>SCRAP_SIZE:
  raise Exception("SCRAP not big enough! Need %d bytes."%(2*len(vars)))
...
f=open(fo,'w')
#f.write(cmpld.code)
#f.write((" "+ast.code).replace(" \\",'\n'))
f.write(s)
f.close()
print("Output Z80 code to %s" % fo)
#print("%d bytes" % cmpld.size)
